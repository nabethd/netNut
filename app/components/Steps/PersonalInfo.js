...
const PersonalInfo = ({nextStep, handleFormDataChange, data}) => {
    ...
    const validate = () => {
        ...
        tempErrors.email = formData.email ? (/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/).test(formData.email) ? '' : 'Email is not valid.' : 'This field is required.';
        tempErrors.phone = formData.phone ? (/^\d+$/).test(formData.phone) ? '' : 'Phone number is not valid.' : 'This field is required.';
